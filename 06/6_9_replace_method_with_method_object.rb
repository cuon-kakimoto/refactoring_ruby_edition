...
# メソッドからメソッドオブジェクトへ。
# [MEMO]
# - これがrailsのvalueオブジェクト的な感じだな。
# - でも、selfが入っているから。ちょっと拡張されている。
# - valueはひとつの概念をオブジェクトにしているけど、これは、ひとつのメソッドをオブジェクトにしている。
# - 設計というよりは、リファクタリングのためだから毛色が違う。
# - ここでもローカル変数をdisる。これがあると、リファクタリングがしにくいせいだな。
# - クラスの値は、元のオブジェクトとメソッドのないの引数、一時変数か。
########################################
...
# HACK: important_value1をメソッド抽出するといろいろいらんくなるね！
class Gamma
  attr_reader :account
